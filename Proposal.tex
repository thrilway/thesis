...
``We drank the teapot dry.''
\bg.* Il a march\'e les jambes raides.\\
He has walked the legs stiff\\
``He walked his legs off.'' \parencite{washio1997resultatives}
\bg.* Gianni ha martellato il metallo piatto. \\
Gianni has hammered the metal flat\\
``Gianni hammered the metal flat.'' \parencite[Napoli 1992, cited by][]{snyder2001nature}
\z.
...
The grammar I hope to describe, schematized below in \Next,  will be a single property of the lexicon which is associated in principle with both endocentric compounding (making it learnable from the PLD) and adjectival resultatives. 
\begin{figure}[h]
  \figex
...
Furthermore, Baker's UTAH is problematic given the growing concensus that ``lexical heads'' are in fact decomposable into acatagorial roots and category determining heads.
...
\end{figure}
I will make the simplfying assumption that theme arguments are merged with the $v$P (Comp V as shorthand) and goals are introduced by adjunction.
