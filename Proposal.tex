...
\textbf{Abstract:}
The fact that languages vary with respect to the resultative interpretation of secondary predication (SP) presents an acquisiton puzzle.
For instance, English and English-type languages allow resultative SP, while Italian and Italian-type languages do not.
\ex.
...
Since the variation is in the interpretation, rather than the surface form, it is not directly learnable from the primary linguistic data.
This thesis shows how such variation could be acquired indirectly.

I take Snyder's (\citeyear{snyder1995language}) generalization, which states that only languages that allow productive compounding allow resultative SP, as a starting point.
However, Snyder's generalization falls short of explanatory adequacy for two reasons:
First, it is stated as a GB-style parameter, which must be reformulated in minimalist terms.
Second, Snyder offers no plausible proposal for how compounding and resultative SP might be linked.
This thesis addresses these concerns and offers an explanation of Snyder's generalization

In Chapter 2, I discuss Snyder's Compounding Parameter and reformulate it as a lexical parameter in line with the Borer-Chomsky Conjecture.
Specifically I propose that those languages that allow compounding also allow bare categories, while those that disallow compounding have only agreeing categories.

Chapter 3 develops a structural analysis of adjectival resultatives based on Kratzer's (\citeyear{kratzer_building_2004}) analysis, but adapted to fit with syntactic theories such as UTAH \parencite{baker1997thematic} and the Movement Theory of Control \parencite{hornstein1999movement}.
Under this analysis, the object of an adjectival resultative (\textit{e.g.} \textit{the metal} in \textit{hammer the metal flat}) is base generated in a small clause adjunct ([ \textit{res}$^\circ$ [ \textit{the metal}, \textit{flat}]]) and undergoes sideward movement to the verbal theme position.
This movement step, I argue, is blocked in Italian-type languages.

Chapter 4 introduces \citeauthor{chomsky2013problems}'s (\citeyear{chomsky2013problems,chomsky2015problems}) label theory, which says that phrase markers are generated without labels and must be labeled upon Transfer for proper interpretation at the CI interface.
I modify Chomsky's proposals in two ways:
First, I adopt Chametzky's (\citeyear{chametzky1996theory}) proposal that adjunction structures are unlabeled.
Second, I propose that labels are interpreted as instructions for semantic composition.

In Chapter 5, I show how resultative SP or the ban on resultative SP can be derived based on the theoretical assumptions proposed and argued for in preceding chapters.
Chapter 6 extends my analysis to directionalized locatives, and Chapter 7 discusses residual issues, and concludes.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[baseline]
    ...
    \node (pld) at (8,0) {PLD};
    \draw [<->] (lex.north east) -- (comp.west);
    \draw [<->] (lex.south east) -- (res.west);
    \draw [->] (pld)--(comp);
  \end{tikzpicture}
...
Beyond Chomsky's arguments in favour of label theory, there are practical reasons to believe that it will provide an explanation of the parametric variation to be investigated in this thesis.
