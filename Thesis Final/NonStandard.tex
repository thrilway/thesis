...
Under a free Merge framework, the computational cost of Move is lowered to that of Merge, while, under a constrained Merge framework, the computational cost of Merge is increased to that of Move.
Note that under both frameworks, the computational costs of Move and Merge are equal to each other.
...
And so on.
I, for one, don't have the slightest clue how to proceed in answering these questions.
Perhaps I am suffering from what Elizabeth Cowper calls ``a poverty of imagination,'' but if I am, then so are the constrained-Merge theorists, as there seems to be no serious attempts by them to address or even entertain these questions.
...
\subsection{``Syntax''}
The term \textit{syntax} has (at least) three interpretations which seem to be used in generative grammar circles.
