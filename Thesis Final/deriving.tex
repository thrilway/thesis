...
\begin{itemize}
    \item Build the Small Clause.
    \item Merge(res, $\beta$)
    \item Copy(DP) + Merge(DP, $\gamma$)
    \item Transfer($\beta$)
        \begin{itemize}
            \item res is a phase head.
            \item Label($\alpha$) = adj
            \item Label($\beta$) = Label($\alpha$)
            \item $\beta$ converges.
        \end{itemize}
    \item Copy(DP)
      \begin{itemize}
	\item Generally, it seems that \textit{overt} specifiers of adjuncts are barred. (See appendix for explanation).
      \end{itemize}
\end{itemize}
\textbf{Step 2: Derive the VP}\\\label{step-2-derive-the-vp}
\begin{itemize}
    \item Merge(DP, {\rm hammer})
        \begin{itemize}
            \item A copy of the DP was in ``working memory''
        \end{itemize}
    \item Merge($\zeta$, $\delta$)
    \item Merge(AgrO, $\eta$)
    \item Copy(DP) + Merge(DP, $\kappa$)
    \item \ldots
    \item Label($\eta$) = $\emptyset$
    \item $\llbracket\langle\eta,\emptyset\rangle\rrbracket =$\\
      $\llbracket\text{hammer the metal}\rrbracket(e) \& \llbracket res\text{ the metal flat}\rrbracket(e)$
\end{itemize}
