...
Since types are rigid in this theory, the need for types to match must be a diving force of syntax.

The second theory, that of \textcite{partee1987noun}, largely agrees with \textcite{heimkratzer1998semantics}, but proposes that the type of a DP can be shifted in order to get the appropriate interpretation.
So, a given DP can be interpreted referentially (type $e$), as a predicate (type $\langle e,t\rangle$), or as a generalized quantifier (type $\langle\langle e,t\rangle, t\rangle$) depending on, among other things, its structural position in a sentence. 
In this theory, while there must be some lexical basis for the semantic type of an expression, the actual use of an expression determines its type.
This means that syntax is less-driven by a need for types to match up.
