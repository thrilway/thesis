...
This suggestion, however, runs into problems almost immediately due to the fact that there are phrases which can be arguments, predicates, or adjuncts as in \crefrange{ex:Adjective}{ex:ProgP}.
...
From the view of NS, then, pair-Merge does a lot of work to no effect; the object derived in \cref{ex:pmerge-deriv} is syntactically indistinguishable from the object derived in \cref{ex:smerge-deriv}.
...
\textbf{Note:} Since the term \textit{operator} already has a particular meaning in generative syntax, I will refer to the operators of predicate logic as \textit{l-operators}.
...
The class of labelled SOs and that of complex expressions of predicate logic can each be divided into three sub-classes.
...
\end{figure}
The next stages of the derivation requires the DP \textit{Joanna} to move out of the AC, leaving us with an unlabellable structure as per the discussion immediately preceding this subsection.
...
Since the AC in \cref{fig:ProgPStruct} is adjoined to VP in this case, and, by hypothesis, the labelling algorithm operates only on the host of a host-adjunct structure, then the AC is not processed by the labelling algorithm.
