...
Suppose hypothesize that the logical form in \cref{ex:AdHocLF} is final logical form of the VP in \cref{ex:ACC-ing}.
\ex.\label{ex:AdHocLF}
$
\begin{array}[t]{rcl}
...
This obviously wouldn't do for the case in which the present participle is the main verb of the clause (\textit{e.g.}, \textit{The dog is running.}), so we need to hypothesize a new head which I will call Prog$_{\Theta}$, and which has the denotation in \cref{ex:ProgAdHoc}.
...
\ex.? \label{ex:DoubleBen} Toby baked Kevin a cake for Oscar.\footnote{
	Note this sentence is acceptable if Kevin is the recipient and Oscar is the benificiary or vice-versa, but not if Kevin and Oscar have the same role.
}
...
I could, therefore, cut this discussion short right here and declare the proposed analysis in \cref{ex:AdHocLF} irreparably flawed.
I have other arguments against the analysis though, so I will present those below.
...
The issue with this hypothesis is that it requires that the ACC-ing clause not be an adjunct.
...
\end{figure}
This structure, however, is untenable without significant additional stipulation for two reasons, which I discuss below.
...
Stipulation (\textit{ii}), requires us to stipulate a landing site for the movement operation; one that ensure the proper word order.
This new landing site implies a new functional head, whose identity will likely be stipulated.
As with stipulation (\textit{i}), this stipulation likely begets more stipulations.\footnote{
	...
	I would argue, however, that these hypotheses do not raise new areas of inquiry, but rather rescue us from raising new areas of inquiry.
...
The level of stipulation required in order to label the structure in \cref{fig:AdHocTree2}, I believe, renders in untenable.
...
In view of the issues with this proposed analysis, I will set it aside.

Which brings us to our final possibility: encoding the \textsc{theme2} on a specialized functional head $\Theta$.
...
\ex. \textsc{sem}$(\Theta) = \lambda P_{\langle s,t\rangle} \lambda e \exists e^{\prime}\left[ P(e)\,\&\,\textsc{theme2}(e)(e^{\prime}) \right]$\label{ex:ThetaSEM1} (First Pass)
...
This is far too stron of an assertion; \cref{ex:ACC-ing} is compatible with only seeing a sliver of the running event.
...
\ex. \textsc{sem}$(\Theta) = \lambda P_{\langle s,t\rangle} \lambda e^{\prime} \exists e\left[ P(e)\,\&\,\textsc{theme2}(e)(e^{\prime}) \right]$\label{ex:ThetaSEM2} (second Pass)
...
This no longer has the flaws of the denoation in \cref{ex:ThetaPSEM12}, but it has introduced a new one.
...
Note that we are only tied to the analysis in \cref{ex:VPProgPSEM} only insofar as we adopt the admittedly simplistic semantic analysis of progressive aspect that we do adopt.
I leave that as a task for more interested readers.
\end{document}
