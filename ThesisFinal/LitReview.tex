...
The second argument against direct learning of resultatives is that even if children could distinguish between depictives and resultatives, there would be no way to rule out resultatives on the basis of PLD.
To make this argument, I will assume a reasonable and, I believe, quite commonly assumed model of parameter setting according to which learners begin with a default parameter setting which they change only in the face of evidence in the PLD. 
I will consider the two logically possible instantiations of this model with respect to the resultative parameter.
The first instantiation, which is doomed to failure, is the one in which resultatives are allowed by default.
The alternative, in which resultatives are disallowed by default, seems to fare better, but I will argue that, in fact, this alternatives faces the same problems that doomed the first alternative.

Suppose the default state of the language faculty is one which is able to generate both resultatives and depictives.
A child in this state, upon hearing an utterance of a secondary predication construction will hypothesize both types of structural descriptions for this utterance and converge on only the one whose interpretation is appropriate in the context of utterance.
This requires a child to make the inductive leap from a series of non-resultative utterances to a ban on resultatives.


Now, consider the alternative, in which the French setting is the default.
How is an English-learner to decide that the resultative structure is an available parse for a secondary predication construction?
Presumably, such a leap will take place if the child hears a secondary predication utterance in a context that only supports a resultative interpretation.
This would activate some sort of fail-safe, which adds resultatives to the grammar.
If this is the case, then the child had resultatives in her grammar all along.
Perhaps resultatives were a dispreferred option, but an option nonetheless, and if this option is available to all children is it still available to adult speakers of a non-resultative language?
If it is available, then why can a French-speaker not acquire resultatives?
If it is no longer available then we have the same question that led us to reject the first alternative: What evidence is needed to reject the resultative option?
