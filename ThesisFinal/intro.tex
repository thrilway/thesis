...
To demonstrate this ability, \textcite{chomsky<++>} distinguishes between two cases of Merge: external and internal.
...
While the Y Model and Merge seem to be the only instances of true consensus among minimlaist syntacticians, there is growing accord about the underlying representation of certin types of word.
Specifically, many minimalist syntacticians now assume that a lexical word, like the noun \textit{chair}, minimally consists of an acategorial root and a categorizing head \parencite{borer2005name,<++>}.
This is commonly expressed in the formalism of a vocabulary insertion rule from the theory of Distributed Morphology \parencite{<++>}<++> as in \cref{ex:VIRule}.
